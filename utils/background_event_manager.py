...
try:
    from gold.telegram_notifier import send_startup_summary
    from gold.xauusd_event_finder import (MONITOR_SLOW_SEC, compute_gold_levels,
                                     fetch_latest_data, read_monitor_interval)
    from gold.coindcx_gold import is_gold_weekend
except ImportError:
...
def load_config() -> dict:
    ...
    for path in (GOLD_CONFIG, STOCK_CONFIG):
        ...
        try:
            ...
            if path == STOCK_CONFIG and "run_times" in raw_yml:
            ...
            if run_stock:
                ...
                if current_hhmm in run_times:
                    if last_executed_stock_slot != current_slot:
                        latest_html = os.path.join("stock", "data", "screener_output", "latest.html")
                        already_ran = False
                        ...
                        if not already_ran:
                            ...
                            subprocess.run(
                                [sys.executable, str(_ROOT / "stock" / "scheduler_run.py")],
                                cwd=str(_ROOT), check=False,
                            )
                        else:

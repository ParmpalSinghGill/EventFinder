...
RULES (all tunable via CLI flags):
  ...
  Pivot HIGH (resistance / upper level) at candle i, price = High[i]:
     ...
     * big-body : >=2 lower-high candles on one side AND the adjacent candle on
                  ...
                  body) and is itself lower, OR
     * left-gap : the next candle has a LOWER high AND the previous candle's
                  high differs by more than left_gap (default 1%) from High[i].
  Pivot LOW (support / lower level): mirror image using Low[] and HIGHER-low
  candles (left-gap uses the previous Low vs Low[i]).
...
def find_labels(df: pd.DataFrame, left: int = 3, right: int = 2,
                ...
                avg_window: int = 20, left_gap: float = 0.01) -> list:
    ...
    def gap_ok(i, arr, after_lower: bool) -> bool:
        """Next candle confirms the turn; previous candle is >left_gap away."""
...
def main():
    ...
    p.add_argument("--outdir", default="stock/data/plots")
    ...
    p.add_argument("--left-gap", type=float, default=0.01,
                   help="Also form a pivot if the next candle confirms the turn "
                        "and the previous candle differs by more than this fraction "
                        "(default 0.01 = 1%).")

...
import stock.screen_levels as screen_levels
...
from utils.project_root import LIVE_EVENT_CONFIG
...
def load_config(path="stock/config.yml"):
    with open(_abs(path), "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f) or {}

    # Live on/off switches written by the control dashboard.
    json_path = str(LIVE_EVENT_CONFIG)
...
def check_and_run_moneycontrol(cfg, now):
    ...
    if should_run:
        ...
        script_path = os.path.join(BASE, "stock", "scrape_moneycontrol_stocks.py")
        try:
...
def main():
    ...
    p.add_argument("--config", default="stock/config.yml")

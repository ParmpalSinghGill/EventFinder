...
BAT = os.path.join(STOCK, "run_screener.bat")
MC_BAT = os.path.join(STOCK, "run_moneycontrol.bat")
VBS = os.path.join(STOCK, "run_screener.vbs")
MC_VBS = os.path.join(STOCK, "run_moneycontrol.vbs")
LOGDIR = os.path.join(STOCK, "data", "screener_output")
STARTUP_DIR = os.path.join(os.environ.get("APPDATA", ""),
                           r"Microsoft\Windows\Start Menu\Programs\Startup")
STARTUP_VBS = os.path.join(STARTUP_DIR, "EventFinder_Startup.vbs")
OPEN_BAT = os.path.join(STOCK, "open_latest.bat")
LATEST_HTML = os.path.join(LOGDIR, "latest.html")
PROTO_KEY = r"HKCU\Software\Classes\eventfinder"   # clickable-toast handler


def load_times():
    with open(os.path.join(STOCK, "config.yml"), "r", encoding="utf-8") as f:
...
def write_bat():
    ...
    content = (
        "@echo off\r\n"
        f'cd /d "{BASE}"\r\n'
        f'"{PYEXE}" "{os.path.join(BASE, "stock", "scheduler_run.py")}" '
...
def write_moneycontrol_bat():
    ...
    content = (
        "@echo off\r\n"
        f'cd /d "{BASE}"\r\n'
        f'"{PYEXE}" "{os.path.join(BASE, "stock", "scrape_moneycontrol_stocks.py")}" '
...
def register_protocol():
    ...
    export_py = os.path.join(BASE, "stock", "export_watchlist.py")
    sched_py = os.path.join(BASE, "stock", "scheduler_run.py")
...
def run_now():
    ...
    subprocess.run([PYEXE, os.path.join(BASE, "stock", "scheduler_run.py"), "--force"])
